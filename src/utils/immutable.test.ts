...
import { number, object, string } from '../shared/libs/validator/validator';
...
describe('Immutable tests', () => {
    ...
    test('parseValidate() - schema validation', () => {
        ...
        // Validation error
        throws(() => Immutable.parseValidate(object({ name: string().min(5) }), '{"name": "Bob"}'));
    ...
    test('freeze() - freezing existing objects', () => {
        ...
        strictEqual(Object(frozen2).push, undefined);
        strictEqual(Object(frozen2).pop, undefined);
        ...
        // Mutating methods are undefined
        strictEqual(Object(frozen1).set, undefined);
        strictEqual(Object(frozen1).deleteProperty, undefined);
    });
});
