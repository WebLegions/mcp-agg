/** biome-ignore-all lint/suspicious/noExplicitAny: Types from Juris codebase */
...
type AsyncCapable<T> = T extends (...args: any[]) => any ? (...args: Parameters<T>) => MaybeAsync<ReturnType<T>> : MaybeAsync<T>;
...
interface BackgroundProperties {
    ...
    backgroundBlendMode?: ReactiveValue<
        | 'normal'
        | 'multiply'
        | 'screen'
        | 'overlay'
...
interface BorderProperties {
    ...
    borderBottomStyle?: ReactiveValue<
...
interface AnimationProperties {
    ...
    animationTimingFunction?: ReactiveValue<
...
interface TransitionProperties {
    ...
    transitionTimingFunction?: ReactiveValue<
        'linear' | 'ease' | 'ease-in' | 'ease-out' | 'ease-in-out' | 'step-start' | 'step-end' | string
    >;
...
interface LayoutProperties {
    ...
    // Vertical align
    verticalAlign?: ReactiveValue<
        'baseline' | 'sub' | 'super' | 'text-top' | 'text-bottom' | 'middle' | 'top' | 'bottom' | string | number
    >;
}
...
interface OutlineProperties {
    ...
    outlineStyle?: ReactiveValue<
        'none' | 'auto' | 'solid' | 'dashed' | 'dotted' | 'double' | 'groove' | 'ridge' | 'inset' | 'outset'
    >;
...
interface SVGProperties {
    ...
    dominantBaseline?: ReactiveValue<
...
type StyleValue = string | CSSProperties | ExtendedStyleObject;
...
export namespace JurisVDOM {
    ...
    export type Element =
        ...
        | { template: ContainerElement }
        | { slot: ContainerElement }
        | RegisteredComponentElements // Allow registered components
        | DynamicElement; // Allow dynamically registered components
}
...
export type JurisComponentFunction<P = Record<string, any>, TState = any> = (
    props: P,
    context: JurisContext<TState>,
) =>
    | JurisVDOMElement
...
// biome-ignore lint/complexity/noBannedTypes: TState default must be flexible object type
...
// biome-ignore lint/style/useNamingConvention: Juris is an external library convention
...
declare global {
    interface Window {
        ...
        // biome-ignore lint/style/useNamingConvention: JurisTypes is an external library convention
        JurisTypes: {
            info: {
                // biome-ignore lint/style/useNamingConvention: External library type names
                Context: string;
                // biome-ignore lint/style/useNamingConvention: External library type names
                VDOMElement: string;
                // biome-ignore lint/style/useNamingConvention: External library type names
        ...
        // biome-ignore lint/style/useNamingConvention: JurisTypeHelpers is an external library convention
        JurisTypeHelpers: {
