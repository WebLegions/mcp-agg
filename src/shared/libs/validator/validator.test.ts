...
describe('Validator', () => {
    ...
    test('should validate multipleOf, step, finite, and safe for numbers', () => {
        ...
        throws(() => mult5.parse(7), /7 undevided by 5/);

        strictEqual(number().step(3).parse(0), 0);
        throws(() => number().step(3).parse(5), /5 undevided by 3/);
    ...
    test('should validate gt, gte, lt, lte comparisons with bigints', () => {
        ...
        throws(() => gt10.parse(10n), /10 equal or smaller than expected \(10\)/);

        strictEqual(bigint().gte(10n).parse(10n), 10n);
        throws(() => bigint().gte(10n).parse(9n), /9 smaller than expected \(10\)/);

        strictEqual(bigint().lt(10n).parse(9n), 9n);
        throws(() => bigint().lt(10n).parse(10n), /10 equal or larger than expected \(10\)/);

        strictEqual(bigint().lte(10n).parse(10n), 10n);
        throws(() => bigint().lte(10n).parse(11n), /11 larger than expected \(10\)/);
    });

    test('should validate positive, negative, nonnegative, nonpositive for bigints', () => {
        strictEqual(bigint().positive().parse(1n), 1n);
        throws(() => bigint().positive().parse(0n), /0 equal or smaller than expected \(0\)/);

        strictEqual(bigint().negative().parse(-1n), -1n);
        throws(() => bigint().negative().parse(0n), /0 equal or larger than expected \(0\)/);

        strictEqual(bigint().nonnegative().parse(0n), 0n);
        throws(() => bigint().nonnegative().parse(-1n), /-1 smaller than expected \(0\)/);

        strictEqual(bigint().nonpositive().parse(0n), 0n);
        throws(() => bigint().nonpositive().parse(1n), /1 larger than expected \(0\)/);
    });

    test('should validate multipleOf and step for bigints', () => {
        const mult5 = bigint().multipleOf(5n);
        strictEqual(mult5.parse(10n), 10n);
        throws(() => mult5.parse(7n), /7 undevided by 5/);

        strictEqual(bigint().step(3n).parse(0n), 0n);
        throws(() => bigint().step(3n).parse(5n), /5 undevided by 3/);
    });
    ...
    test('should validate NaN and literal values (string, number, boolean, null, undefined)', () => {
        ...
        strictEqual(voidVal().parse(undefined), undefined);
        throws(() => voidVal().parse(null), /Expected literal undefined, got null/);
    });
    ...
    test('should work with validation methods', () => {
        ...
        throws(() => withDefault.parse(-1), /-1 equal or larger than expected \(0\)/);
    ...
    test('should validate and transform valid objects', () => {
        ...
        throws(() => parse(obj, undefined));
        throws(() => parse(obj, 42));
    });

    test('should throw for invalid objects', () => {
        throws(() => parse(object({ name: string().min(2) }), { name: 'J' }));
        throws(() => parse(object({ age: number().int().min(0) }), { age: -1 }));
        throws(() => parse(object({ age: number().int() }), { age: 25.5 }));
        throws(() => parse(object({ email: string().email() }), { email: 'invalid' }));
        throws(() => parse(object({ name: string(), age: number() }), { name: 'John' })); // missing required
    });
    ...
    test('should handle optional and required fields', () => {
        ...
        deepEqual(parse(object({ name: string() }), { name: '' }), { name: '' });
    ...
    test('should coerce types and validate formats', () => {
        ...
        throws(() => parse(object({ age: number() }), { age: 'invalid' }));
    ...
    test('should validate array of objects and deeply nested structures', () => {
        ...
        throws(() => parse(obj4, { users: [{ age: 30 }] }));
    ...
    test('should allow multiple custom validators', () => {
        ...
        strictEqual(parse(schema, { score: 25 })?.score, 25);
        throws(() => parse(schema, { score: 150 }));
    });
    ...
    test('safeParse should return Zod-compatible result format', () => {
        ...
        strictEqual(result1.success, true);
        if (result1.success) {
            strictEqual(result1.data, 'hello');
        }
        ...
        if (!result2.success) {
            ...
            ok(result2.error.message.includes('at least 3'));
        }
        ...
        strictEqual(result3.success, true);
        if (result3.success) {
            strictEqual(result3.data.name, 'John');
            strictEqual(result3.data.age, 30);
        }

        // Test with invalid object
        const result4 = objSchema.safeParse({ name: 'Jane', age: 'invalid' });
        strictEqual(result4.success, false);
        if (!result4.success) {
            ok(result4.error instanceof Error);
        }
    });
...
test('should extend object schemas', () => {
    ...
    throws(() => extendedSchema.parse({ name: 'John', age: 30, email: 'invalid' }), /email/);
});
...
test('should extend object with strict mode preserved', () => {
    ...
    throws(() => extended.parse({ name: 'John', age: 30, extra: 'value' }));
});
...
test('should get defs from NaN validator', () => {
    ...
    ok(defs.description?.includes('NaN'));
    ok(defs.not !== undefined);
});
...
test('should set and retrieve examples from validators', () => {
    ...
    deepStrictEqual(emailDefs.examples, ['user@example.com']);
    strictEqual(emailDefs.format, 'email');

    const urlValidator = string().url();
    const urlDefs = urlValidator.defs();
    deepStrictEqual(urlDefs.examples, ['https://example.com']);
    ...
    deepStrictEqual(customDefs.examples, ['admin@test.com', 'support@test.com']);

    // Test examples on plain string
    const plainValidator = string().examples('foo', 'bar', 'baz');
    const plainDefs = plainValidator.defs();
    deepStrictEqual(plainDefs.examples, ['foo', 'bar', 'baz']);
    ...
    deepStrictEqual(chainedDefs.examples, ['hello', 'world']);
    strictEqual(chainedDefs.minLength, 3);
    strictEqual(chainedDefs.maxLength, 10);
});
