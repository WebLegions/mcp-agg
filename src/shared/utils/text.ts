/**
 * Slugifies a string by converting it to lowercase and replacing spaces/special chars with hyphens.
 * @param args - Strings to slugify.
...
export function slugify(...args: string[]): string {
    return args
        .join('-')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-') // Replace non-alphanum with a single hyphen
        .replace(/^-+|-+$/g, ''); // Trim leading/trailing hyphens
}

/**
 * Dotifies a string by converting it to lowercase and replacing spaces/special chars with dots.
 * @param args - Strings to dotify.
 * @returns The dotified string.
 */
export function dotify(...args: string[]): string {
    return args
        .join('.')
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '.') // Replace non-alphanum with dot
 ...
 * Examples: name -> Name, serverUrl -> Server URL, max_length -> Max Length
 * @param fieldName - The field name to convert to a label.
 * @returns The human-readable label.
 */
export function labelify(fieldName: string): string {
    return fieldName
