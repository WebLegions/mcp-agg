...
describe('text utils', () => {
    test('slugify', () => {
        ...
        strictEqual(slugify('ThisIsATitle!'), 'thisisatitle');
        ...
        strictEqual(slugify('Hello', 'World'), 'hello-world');
        strictEqual(slugify('##foo', 'bar', 'baz'), 'foo-bar-baz');
        strictEqual(slugify('foo_bar', 'baz qux'), 'foo-bar-baz-qux');
        strictEqual(slugify('foo--bar', 'baz'), 'foo-bar-baz');
        strictEqual(slugify('foo@bar!', 'baz#qux'), 'foo-bar-baz-qux');
    ...
    test('dotify with dotNotation option', () => {
        ...
        strictEqual(dotify('serverModal', 'name'), 'servermodal.name');
        strictEqual(dotify('ui', 'showServerModal'), 'ui.showservermodal');
        strictEqual(dotify('servers', 'items'), 'servers.items');
        strictEqual(dotify(' -- serverModal  ', '  name  '), 'servermodal.name');
        strictEqual(dotify('na--me'), 'na.me');
    });

    test('generates human-readable labels from field names', () => {
        strictEqual(labelify('name'), 'Name');
        strictEqual(labelify('serverUrl'), 'Server Url');
        strictEqual(labelify('max_length'), 'Max Length');
        strictEqual(labelify('min-value'), 'Min Value');
        strictEqual(labelify('--firstName'), 'First Name');
        strictEqual(labelify('user_email_address'), 'User Email Address');
        strictEqual(labelify('APIKey'), 'A P I Key');
    });
    ...
    test('escapes special HTML characters', () => {
        strictEqual(escapeHTML('<div>'), '&lt;div&gt;');
        strictEqual(escapeHTML('&"\'<>'), '&amp;&quot;&#x27;&lt;&gt;');
        strictEqual(escapeHTML('plain'), 'plain');
        strictEqual(escapeHTML(''), '');
    });
